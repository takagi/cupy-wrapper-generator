...
def cupy_name(node, env):
    ...
    cupy_name_ = _environment.function_cupy_name(cuda_name, env)
    if cupy_name_ is not None:
        return cupy_name_
    pattern = _environment.function_pattern(env)
...
def hip_name(node, env):
    cuda_name = _pycparser.function_name(node, strip_suffix=True)
    hip_node = _environment.function_hip_node(cuda_name, env)
